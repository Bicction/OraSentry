...
collect_db_security() {
    ...
    if [[ "${HAS_USERS_ORACLE_MAINTAINED:-NO}" == "YES" && "${HAS_ROLES_ORACLE_MAINTAINED:-NO}" == "YES" ]]; then
        ...
        exec_sql "${d}/sys_privs.txt" "WITH business_users AS (SELECT username FROM dba_users WHERE username NOT IN ('ANONYMOUS','APEX_PUBLIC_USER','APPQOSSYS','AUDSYS','CTXSYS','DBSFWUSER','DBSNMP','DIP','DVF','DVSYS','EXFSYS','FLOWS_FILES','GGSYS','GSMADMIN_INTERNAL','GSMCATUSER','GSMUSER','LBACSYS','MDDATA','MDSYS','OJVMSYS','OLAPSYS','ORACLE_OCM','ORDDATA','ORDSYS','OUTLN','REMOTE_SCHEDULER_AGENT','SI_INFORMTN_SCHEMA','SPATIAL_CSW_ADMIN_USR','SPATIAL_WFS_ADMIN_USR','SYS','SYS\$UMF','SYSBACKUP','SYSDG','SYSKM','SYSRAC','SYSTEM','WMSYS','XDB','XS\$NULL')), principals AS (SELECT username principal FROM business_users UNION SELECT granted_role FROM dba_role_privs WHERE grantee IN (SELECT username FROM business_users) AND granted_role NOT IN ('CONNECT','RESOURCE','DBA','CTXAPP','SELECT_CATALOG_ROLE','EXECUTE_CATALOG_ROLE','EXP_FULL_DATABASE','IMP_FULL_DATABASE','DATAPUMP_EXP_FULL_DATABASE','DATAPUMP_IMP_FULL_DATABASE','SCHEDULER_ADMIN','GSMADMIN_ROLE','GSMUSER_ROLE','SYSUMF_ROLE')) SELECT DISTINCT p.grantee, p.privilege, p.admin_option FROM dba_sys_privs p JOIN principals x ON x.principal=p.grantee ORDER BY p.grantee, p.privilege;"
    fi
    ...
    if [[ "${HAS_USERS_ORACLE_MAINTAINED:-NO}" == "YES" ]]; then
        exec_sql "${d}/public_risky_grants.txt" "SELECT p.owner, p.table_name, p.privilege, p.grantable FROM dba_tab_privs p WHERE p.grantee='PUBLIC' AND p.privilege IN ('EXECUTE','READ','WRITE') AND p.table_name IN ('UTL_FILE','UTL_HTTP','UTL_TCP','UTL_SMTP','DBMS_SCHEDULER','DBMS_JOB','DBMS_LOB','DBMS_SQL','DBMS_RANDOM') AND NOT EXISTS (SELECT 1 FROM dba_users u WHERE u.username=p.owner AND u.oracle_maintained='Y') ORDER BY p.owner, p.table_name, p.privilege;"
    else
        exec_sql "${d}/public_risky_grants.txt" "SELECT p.owner, p.table_name, p.privilege, p.grantable FROM dba_tab_privs p WHERE p.grantee='PUBLIC' AND p.privilege IN ('EXECUTE','READ','WRITE') AND p.table_name IN ('UTL_FILE','UTL_HTTP','UTL_TCP','UTL_SMTP','DBMS_SCHEDULER','DBMS_JOB','DBMS_LOB','DBMS_SQL','DBMS_RANDOM') AND p.owner NOT IN ('ANONYMOUS','APEX_PUBLIC_USER','APPQOSSYS','AUDSYS','CTXSYS','DBSNMP','DIP','DVSYS','EXFSYS','FLOWS_FILES','GSMADMIN_INTERNAL','LBACSYS','MDSYS','OJVMSYS','OLAPSYS','ORACLE_OCM','ORDDATA','ORDSYS','OUTLN','SYS','SYSTEM','WMSYS','XDB','XS\$NULL') ORDER BY p.owner, p.table_name, p.privilege;"
    fi
